...
event_markers = {'OBM','EBM','LOBS','FTGO','OBGC','OBEND','LEXT','FTGE','EXGC','EXEND'};

% epoch length in seconds
epoch_length = [-1.8 1.8];
...
for s_idx=1:size(study_info.participant_info,1)
    ...
    end_evts=find(strcmp('EXEND',{EEG.event.type}));    
    for i=1:length(start_evts)
        ...
        EEG.event(end_evts(i)).value=hand;
        EEG.event(end_evts(i)).trial=i;
    end
    %check for consistency and reorder the events chronologically...
    EEG=eeg_checkset(EEG,'eventconsistency');
    
    obs_subj_rows=find(strcmp(subject,obs_df.Subject));
    ...
    end_evts=find(strcmp('OBEND',{EEG.event.type}));    
    for i=1:length(start_evts)
    ...
    for epochIdx = 1:length(EEG.epoch)
        ...
        if length(allEventIdx) == 1 % If there is only 1 event, it must have latency zero.
            ...
            EEG.epoch(epochIdx).eventvalue     = EEG.epoch(epochIdx).eventvalue(zeroLatencyIdx);
            EEG.epoch(epochIdx).eventtrial     = EEG.epoch(epochIdx).eventtrial(zeroLatencyIdx);
        end
    end
